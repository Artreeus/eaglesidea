...
NODE_API_ENDPOINT = 'http://localhost:3000/api/misp-results/sha256/upload'
...
async def fetch_misp_data(session, hashes):
    """Sends a list of SHA256 hashes to the MISP server for searching."""
    try:
        payload = {
            "returnFormat": "json",
            "type": "sha256", # Search for SHA256 attribute type
            "value": hashes
        }
        ...
        print(f"ERROR: Could not fetch MISP data for SHA256 hashes: {str(e)}")
        return {}

async def send_results_to_node(session, results):
    """Posts the formatted scan results back to the Node.js application."""
    if not results:
        print("INFO: No SHA256 hash results to send to Node.js.")
        return
    ...
    try:
        ...
        async with session.post(NODE_API_ENDPOINT, json=payload) as response:
            if response.status == 200:
                print(f"SUCCESS: Sent {len(results)} SHA256 hash results to Node.js.")
            else:
                print(f"ERROR: Failed to send SHA256 results. Status: {response.status}, Response: {await response.text()}")
    except Exception as e:
        print(f"ERROR: Failed to send SHA256 results to Node.js: {str(e)}")


async def main(hashes_to_scan):
    """Main function to process SHA256 hashes and send results."""
    async with aiohttp.ClientSession() as session:
        print(f"INFO: Sending {len(hashes_to_scan)} SHA256 hashes to MISP...")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python misp_sha256_scanner.py <hash1> <hash2> ...")
        sys.exit(1)
